...
# Auxillary functions for Jellyfish from https://github.com/lechengfan/cs244-assignment2
...
def get_path_counts(ecmp_paths, all_ksp, dsp_paths, traffic_matrix, all_links):
    counts = {}
    # initialize counts for all links
    for link in all_links:
        a, b = link
        counts[(str(a),str(b))] = {"8-ksp":0, "8-ecmp": 0, "dsp": 0} 
        counts[(str(b),str(a))] = {"8-ksp":0, "8-ecmp": 0, "dsp": 0} 
    for start_host in range(len(traffic_matrix)):
        ...
        ksp = dsp_paths[(str(start_node), str(dest_node))]
        for path in ksp:
            prev_node = None
            for node in path:
                if not prev_node:
...
def assemble_histogram(path_counts, file_name):
    ...
    dsp_distinct_path_counts = []

    for _, value in sorted(path_counts.items(), key=lambda v: v[1]["8-ksp"]):
        ksp_distinct_paths_counts.append(value["8-ksp"])
    for _, value in sorted(path_counts.items(), key=lambda v: v[1]["8-ecmp"]):
        ecmp_8_distinct_paths_counts.append(value["8-ecmp"])
    for _, value in sorted(path_counts.items(), key=lambda v: v[1]["dsp"]):
        dsp_distinct_path_counts.append(value["dsp"])
    ...
    ax1.plot(x, dsp_distinct_path_counts, color='r', label="8 Diverse Short Paths")
